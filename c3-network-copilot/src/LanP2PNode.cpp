...
namespace lanp2p
{
	...
	void LanP2PNode::stopUdpListen()
	{
		...
		{
			sockaddr_in a{}; a.sin_family = AF_INET; a.sin_port = htons(_discoveryPort); a.sin_addr.s_addr = inet_addr("127.0.0.1");
	...
	void LanP2PNode::stop()
	{
		...
		if (_udpBroadcaster.joinable()) _udpBroadcaster.join();
		if (_udpListener.joinable()) _udpListener.join();
		if (_tcpListener.joinable()) _tcpListener.join();
		if (_maintenanceThread.joinable()) _maintenanceThread.join();
	}

	// 设置回调
	void LanP2PNode::setOnPeerDiscovered(const std::function<void(const PeerInfo&)>& cb) { _onPeerDiscovered = cb; }
	void LanP2PNode::setOnMatchRequest(const std::function<void(const PeerInfo&, const std::string& matchId)>& cb) { _onMatchRequest = cb; }
	void LanP2PNode::setOnMatchResponse(const std::function<void(const PeerInfo&, bool accepted, const std::string& matchId)>& cb) { _onMatchResponse = cb; }
	void LanP2PNode::setOnMatchInterrupted(const std::function<void(const PeerInfo&, const std::string& matchId)>& cb) { _onMatchInterrupted = cb; }
	void LanP2PNode::setOnGameMove(const std::function<void(const PeerInfo&, int x, int y, int z)>& cb) { _onGameMove = cb; }
	...
	std::vector<PeerInfo> LanP2PNode::getPeersSnapshot()
	{
		...
		{
			if (_matchesByKey.find(it->first) != _matchesByKey.end()) { ++it; continue; }
		...
		std::vector<PeerInfo> v; v.reserve(_peersByKey.size());
		for (auto& kv : _peersByKey) v.push_back(kv.second);
		return v;
	}
	...
	void LanP2PNode::udpBroadcastLoop()
	{
		...
		if ((SOCKET)s == INVALID_SOCKET) return;
		setReuse(s); setBroadcast(s);

		// 等待TCP端口绑定完成
		for (int i=0; i<50 && _running && _broadcastActive && !_tcpBoundReady.load(); ++i)
			std::this_thread::sleep_for(std::chrono::milliseconds(20));

		sockaddr_in addrBC{}; addrBC.sin_family = AF_INET; addrBC.sin_port = htons(_discoveryPort); addrBC.sin_addr.s_addr = INADDR_BROADCAST;
		sockaddr_in addrLoop{}; addrLoop.sin_family = AF_INET; addrLoop.sin_port = htons(_discoveryPort); addrLoop.sin_addr.s_addr = inet_addr("127.0.0.1");

		// 初始快速广播5次
		for (int b =0; b <5 && _running && _broadcastActive; ++b)
		{
			char buf[256]; int len;
			if (_nodeName.empty()) len = std::snprintf(buf, sizeof(buf), "DISC|%s|%u", _nodeId.c_str(), (unsigned)_tcpPort);
			else len = std::snprintf(buf, sizeof(buf), "DISC|%s|%u|%s", _nodeId.c_str(), (unsigned)_tcpPort, _nodeName.c_str());
	...
	void LanP2PNode::udpListenLoop()
	{
		...
		if ((SOCKET)s == INVALID_SOCKET) return;
		setReuse(s);
		sockaddr_in addr{}; addr.sin_family = AF_INET; addr.sin_port = htons(_discoveryPort); addr.sin_addr.s_addr = INADDR_ANY;
		if (bind(static_cast<SOCKET>(s), (sockaddr*)&addr, sizeof(addr)) !=0) { closesock(s); return; }
		char buf[512];
		while (_running && _udpListenActive)
		{
			...
			if (!_udpListenActive) break;
			if (r <=0) { std::this_thread::sleep_for(std::chrono::milliseconds(50)); continue; }
			buf[r] =0; std::string line(buf);
			if (line.compare(0,5, "DISC|") ==0)
			{
				size_t p1 = line.find('|',5); size_t p2 = line.find('|', p1 +1);
				if (p1 != std::string::npos && p2 != std::string::npos)
				{
					...
					std::string name; if (p2 != std::string::npos && p2 +1 < line.size()) name = line.substr(p2 +1);
					if (id != _nodeId)
					{
						PeerInfo info; info.id = id; info.name = name; info.ip = ip; info.tcpPort = tcpPort; info.lastSeenMs = nowMs();
						std::string key = ip + ":" + std::to_string(tcpPort) + ":" + id;
						bool notify = false;
						{
							std::lock_guard<std::mutex> lk(_peersMutex);
							// 优先保留局域网IP记录，若存在则忽略相同ID的回环地址
							bool hasNonLoopForId = false; std::string loopKeyToErase;
							for (auto it = _peersByKey.begin(); it != _peersByKey.end(); ++it)
							{
								const PeerInfo& e = it->second;
								if (e.id == id)
								{
									if (e.ip != "127.0.0.1") hasNonLoopForId = true; else loopKeyToErase = it->first;
								}
							}
							...
							{
								if (ip != "127.0.0.1" && !loopKeyToErase.empty()) _peersByKey.erase(loopKeyToErase);
								_peersByKey[key] = info; notify = true;
							}
						}
						if (notify && _onPeerDiscovered) _onPeerDiscovered(info);
					}
				}
			}
		}
	...
	bool LanP2PNode::sendGameMove(const std::string& peerIp, uint16_t peerTcpPort, int x, int y, int z)
	{
		for (int attempt = 0; attempt < _maxSendRetries; ++attempt)
		{
			...
			{ closesock(s); std::this_thread::sleep_for(std::chrono::milliseconds(50)); continue; }
			std::ostringstream oss; oss << "MOVE|" << x << "|" << y << "|" << z << "|";
			bool ok = tcpSendFramed(s, oss.str()); closesock(s);
	...
	void LanP2PNode::tcpListenLoop()
	{
		...
		if (_tcpPort ==0)
		{
			sockaddr_in addr{}; addr.sin_family = AF_INET; addr.sin_port = htons(0); addr.sin_addr.s_addr = INADDR_ANY;
			if (bind(static_cast<SOCKET>(s), (sockaddr*)&addr, sizeof(addr)) ==0)
			{
				int len = sizeof(addr);
				if (getsockname(static_cast<SOCKET>(s), (sockaddr*)&addr, &len) ==0) { chosen = ntohs(addr.sin_port); }
			}
		}
		else
		{
			for (int attempt =0; attempt <128; ++attempt)
			{
				...
				sockaddr_in addr{}; addr.sin_family = AF_INET; addr.sin_port = htons(portTry); addr.sin_addr.s_addr = INADDR_ANY;
				if (bind(static_cast<SOCKET>(s), (sockaddr*)&addr, sizeof(addr)) ==0) { chosen = portTry; break; }
			}
		}
		if (chosen ==0) { closesock(s); return; }
		_tcpPort = chosen; _tcpBoundReady.store(true);

		if (listen(static_cast<SOCKET>(s),8) !=0) { closesock(s); return; }
		while (_running && _tcpActive)
		{
			...
			if ((SOCKET)c == INVALID_SOCKET) { std::this_thread::sleep_for(std::chrono::milliseconds(50)); continue; }
	...
	uint16_t LanP2PNode::findPeerTcpPort(const std::string& ip, const std::string& id)
	{
		...
		for (auto& kv : _peersByKey)
		{
			...
			if (p.ip == ip && p.id == id) return p.tcpPort;
		}
		return 0;
	}
		...
		while (_running && tcpRecvFramed(sock, payload))
		{
			...
			if (payload.compare(0,4, "REQ|") ==0)
			{
				...
				if (p1 != std::string::npos && p2 != std::string::npos && p3 != std::string::npos)
				{
					...
					uint16_t fromPort =0; try { fromPort = (uint16_t)std::stoi(payload.substr(p1 +1, p2 - (p1 +1))); } catch (...) { fromPort =0; }
					std::string matchId = payload.substr(p2 +1, p3 - (p2 +1));
					std::string toId; if (p4 != std::string::npos) toId = payload.substr(p3 +1, p4 - (p3 +1));
					if (fromId == _nodeId) continue; // 忽略自身请求
					if (!toId.empty() && toId != _nodeId) continue; // 目标不是我则忽略
					// 用消息更新/插入对端表
					PeerInfo piMsg; piMsg.id = fromId; piMsg.ip = remoteIp; piMsg.tcpPort = fromPort; piMsg.lastSeenMs = ts;
					{
						std::lock_guard<std::mutex> lk(_peersMutex);
						std::string key = piMsg.ip + ":" + std::to_string(piMsg.tcpPort) + ":" + piMsg.id;
						auto it = _peersByKey.find(key); if (it != _peersByKey.end()) piMsg.name = it->second.name;
			...
			else if (payload.compare(0,5, "RESP|") ==0)
			{
				...
				if (p1 != std::string::npos && p2 != std::string::npos && p3 != std::string::npos)
				{
					...
					PeerInfo pi; pi.id = fromId; pi.ip = remoteIp; pi.tcpPort = ptcp; pi.lastSeenMs = ts;
					{
						std::lock_guard<std::mutex> lk(_peersMutex);
						for (auto& kv : _peersByKey)
						{
							const PeerInfo& pr = kv.second;
							if (pr.ip == remoteIp && pr.id == fromId) { pi.name = pr.name; break; }
						}
					}
					if (_onMatchResponse) _onMatchResponse(pi, accepted, matchId);
					if (!accepted) { clearMatch(remoteIp, ptcp, fromId, matchId, false); }
				}
			}
			else if (payload.compare(0,4, "INT|") ==0)
			{
				// 格式：INT|fromId|matchId|
				size_t p1 = payload.find('|',4); size_t p2 = payload.find('|', p1 +1);
				if (p1 != std::string::npos && p2 != std::string::npos)
				{
					std::string fromId = payload.substr(4, p1 -4); if (fromId == _nodeId) continue;
					std::string matchId = payload.substr(p1 +1, p2 - (p1 +1));
					uint16_t ptcp = findPeerTcpPort(remoteIp, fromId);
					PeerInfo pi; pi.id = fromId; pi.ip = remoteIp; pi.tcpPort = ptcp; pi.lastSeenMs = ts;
					{
					...
					if (_onMatchInterrupted) _onMatchInterrupted(pi, matchId);
			...
			else if (payload.compare(0,3, "HB|") ==0)
			{
				...
				size_t p1 = payload.find('|',3); size_t p2 = (p1!=std::string::npos)?payload.find('|', p1 +1):std::string::npos;
				if (p1 != std::string::npos && p2 != std::string::npos)
				{
			...
			else if (payload.compare(0, 5, "MOVE|") == 0)
			{
				...
				if (p1 != std::string::npos && p2 != std::string::npos && p3 != std::string::npos)
				{
					try
					{
						...
						PeerInfo pi; pi.ip = remoteIp; pi.lastSeenMs = ts;
						{
							std::lock_guard<std::mutex> lk(_peersMutex);
							for (auto& kv : _peersByKey)
							{
								const PeerInfo& p = kv.second;
								if (p.ip == remoteIp) { pi = p; break; }
							}
						}
						if (_onGameMove) { _onGameMove(pi, x, y, z); }
					}
					catch (...) { /* 转换失败忽略 */ }
				}
	...
	bool LanP2PNode::sendMatchRequest(const std::string& peerIp, uint16_t peerTcpPort, const std::string& matchId)
	{
		for (int attempt = 0; attempt < _maxSendRetries; ++attempt)
		{
			...
			sockaddr_in addr{}; addr.sin_family = AF_INET; addr.sin_port = htons(peerTcpPort); addr.sin_addr.s_addr = inet_addr(peerIp.c_str());
			if (connect(static_cast<SOCKET>(s), (sockaddr*)&addr, sizeof(addr)) != 0)
			{ closesock(s); std::this_thread::sleep_for(std::chrono::milliseconds(100)); continue; }
			std::string toId;
			{
				std::lock_guard<std::mutex> lk(_peersMutex);
				for (auto& kv : _peersByKey)
				{
					const PeerInfo& p = kv.second; if (p.ip == peerIp && p.tcpPort == peerTcpPort) { toId = p.id; break; }
				}
			}
			std::ostringstream oss;
			if (toId.empty()) oss << "REQ|" << _nodeId << "|" << _tcpPort << "|" << matchId << "|";
			else oss << "REQ|" << _nodeId << "|" << _tcpPort << "|" << matchId << "|" << toId << "|";
			bool ok = tcpSendFramed(s, oss.str()); closesock(s);
			if (ok) { if (!toId.empty()) markMatchActive(peerIp, peerTcpPort, toId, matchId); return true; }
	...
	bool LanP2PNode::respondToMatch(const std::string& peerIp, uint16_t peerTcpPort, const std::string& matchId, bool accept)
	{
		for (int attempt = 0; attempt < _maxSendRetries; ++attempt)
		{
			...
			bool ok = tcpSendFramed(s, oss.str()); closesock(s);
			if (ok) return true; std::this_thread::sleep_for(std::chrono::milliseconds(100));
		}
		return false;
	}
	...
	{
		std::random_device rd; std::mt19937_64 rng(rd()); std::uniform_int_distribution<uint64_t> dist; uint64_t v = dist(rng);
		char buf[17]; std::snprintf(buf, sizeof(buf), "%016llx", (unsigned long long)v); return std::string(buf);
	}
		...
		while (_running && _maintenanceActive)
		{
			...
			std::vector<std::tuple<std::string, std::string, uint16_t, std::string, uint64_t>> matchSnapshot;
			{
				...
				for (auto& kv : _matchesByKey)
				{
					...
					size_t p1 = key.find(':'); size_t p2 = (p1==std::string::npos)?std::string::npos:key.find(':', p1+1);
					std::string ip = (p1==std::string::npos)?std::string():key.substr(0,p1);
					uint16_t port =0; if (p1!=std::string::npos && p2!=std::string::npos) { try { port = (uint16_t)std::stoi(key.substr(p1+1, p2-(p1+1))); } catch(...) { port=0; } }
			...
			for (auto& t : matchSnapshot)
			{
				...
				if (_matchHeartbeatIntervalMs>0 && (now2 - last) >= _matchHeartbeatIntervalMs)
				{
					...
					std::lock_guard<std::mutex> lk(_peersMutex);
					auto it = _matchesByKey.find(key);
					if (it != _matchesByKey.end() && it->second.matchId == matchId) it->second.lastHbMs = now2;
				}
	...
	// 在匹配表中标记对战活跃（用于心跳）
	void LanP2PNode::markMatchActive(const std::string& ip, uint16_t tcpPort, const std::string& peerId, const std::string& matchId)
	{
		std::lock_guard<std::mutex> lk(_peersMutex);
		std::string key = ip + ":" + std::to_string(tcpPort) + ":" + peerId;
		auto& st = _matchesByKey[key]; st.matchId = matchId; st.lastHbMs = nowMs();
	}

	// 清理匹配状态；必要时回调上层中断事件
	void LanP2PNode::clearMatch(const std::string& ip, uint16_t tcpPort, const std::string& peerId, const std::string& matchId, bool notify)
	{
		{
			std::lock_guard<std::mutex> lk(_peersMutex);
			...
			{
				const PeerInfo& p = it->second;
				if (p.ip == ip && p.tcpPort == tcpPort && p.id == peerId) it = _peersByKey.erase(it); else ++it;
			}
		}
